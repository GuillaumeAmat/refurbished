...
export class ScoreScreen {
  ...
  #handleInput() {
    const now = Date.now();
    if (now - this.#debounceTime < ScoreScreen.DEBOUNCE_MS) return;
    ...
    for (const playerId of [1, 2] as PlayerId[]) {
      ...
      const aPressed = input.isButtonPressed('a');
      const bPressed = input.isButtonPressed('b');

      // Left/Right to switch between save/skip
      ...
      // Up/Down to cycle through characters
